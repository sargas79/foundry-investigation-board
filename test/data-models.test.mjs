...
export default async function testDataModels() {
  ...
  await check("round-trips a fully populated clue", () => {
    const c = new ClueData({
  ...
  await check("rejects values outside the declared choices", () => {
  ...
  await check("clamps width and rotation to their bounds", () => {
  ...
  await check("onBoard reflects dismissal", () => {
  ...
  await check("templateConfig resolves, and falls back for an unknown template", () => {
  ...
  await check("applies defaults to a new connection", () => {
  ...
  await check("touches() and other() traverse the string", () => {
    const c = new ConnectionData({from: ID_A, to: ID_B});
  ...
  await check("rejects a malformed clue id", () => {
